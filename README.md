...
> 驾考宝典式 AI 面试备考平台：基于**你的简历**与**Boss 真实在招 JD** 智能出题，每题附参考答案、解析与面试官追问链。

[![License: AGPL v3](https://img.shields.io/badge/License-AGPL_v3-blue.svg)](./LICENSE)

像刷科目一一样备战面试——海量结构化题库 + AI 动态出题 + 错题强化 + 语音模拟面试。
...
| **C · JD 精准定制** | 岗位链接 / 截图 / 粘贴文本 | 抓取 JD + 公司背景，与简历交叉生成定向备战包，含**岗位匹配度报告**（强匹配 / 弱匹配补强 / 缺口应对话术） |
...
- 求职看板：已投递 → 笔试 → 面试 → Offer 四列状态机，面试倒计时联动备战
...
设计要点：队列 + SSE 解决「出题慢」，状态机 + 答案二次验证解决「答案错」，MCP 只读检索解决「数据合规」，分层 LLM + 产物缓存解决「成本贵」。
...
- 简历等个人数据脱敏后才送 LLM，支持一键导出与彻底删除；
- AI 生成的参考答案标注「仅供参考」，客观题采用双模型交叉验证。

## Roadmap

- [ ] v0.1 基础备考闭环：账号 + 引擎 A + 刷题 + 错题本
